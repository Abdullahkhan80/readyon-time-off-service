...
let HcmService = HcmService_1 = class HcmService {
    ...
    constructor(httpService) {
        ...
        this.breaker.fallback((_err) => {
            this.logger.error('HCM Service is down (Circuit Breaker OPEN)');
            throw new common_1.ServiceUnavailableException('HCM System is temporarily unavailable.');
        });
    }
    ...
    async validateBalance(employeeId, hours) {
        try {
            const data = await this.breaker.fire(`/balance/${employeeId}`, 'GET');
    ...
    async deductBalance(employeeId, hours, requestId) {
        try {
            const data = await this.breaker.fire('/deduct', 'POST', { employeeId, hours, requestId });
