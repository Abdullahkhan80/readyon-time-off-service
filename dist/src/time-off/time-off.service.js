...
let TimeOffService = TimeOffService_1 = class TimeOffService {
    ...
    async approveRequest(requestId, dto) {
        ...
        try {
            return await this.dataSource.transaction(async (manager) => {
                const balance = await manager.findOne(time_off_balance_entity_1.TimeOffBalance, { where: { employeeId: request.employeeId, locationId: request.locationId, type: 'PTO' } });
                if (balance) {
    ...
    async reconcileBalances() {
        ...
        return await this.dataSource.transaction(async (manager) => {
            ...
            for (const item of snapshot) {
                let balance = await manager.findOne(time_off_balance_entity_1.TimeOffBalance, {
                ...
                if (balance) {
                    ...
                    let employee = await manager.findOne(employee_entity_1.Employee, { where: { id: item.employeeId } });
                    if (!employee) {
                        employee = manager.create(employee_entity_1.Employee, {
                            id: item.employeeId,
                            firstName: 'Imported',
                            lastName: 'User',
                    ...
                    balance = manager.create(time_off_balance_entity_1.TimeOffBalance, {
